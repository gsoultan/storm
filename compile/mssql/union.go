...
// branch names its own. The shape is standard SQL and carries over; the row cap
// does not, and neither does the ordering's null placement.
//
// What DOES carry over, and could not on MySQL, is a declared parameter used in
// more than one branch. Names bind here, so `@p1` in two branches is one value
// passed once — the property PostgreSQL's `$1` has. compile/mysql refuses that
// outright because position is what binds a value there.
...
//
// The cap is OFFSET/FETCH, which is a clause OF the ordering — so a union with
// no declared ordering still gets one, because there is nowhere else to hang
// the cap. `(SELECT NULL)` is the constant the optimizer discards; ordering by
// an output column instead would be a sort the declaration did not ask for.
func UnionSuffix(u *schema.Union) string {
	...
	if len(u.OrderBy) > 0 {
		...
		b.WriteString(OrderFallback)
	}
	// The cap is the last placeholder, after every declared parameter — the
	// order the generated function takes them.
	b.WriteString(" OFFSET 0 ROWS FETCH NEXT " + Param(len(u.Params)+1) + " ROWS ONLY")
	return b.String()
}

// UnionOrderRefused reports an ordering SQL Server cannot express, or nil.
