...
// type and uses JSON_TABLE over a bound document (ADR-0010). Oracle has no
// array type either, and JSON_TABLE is how one bound document becomes rows —
// statement text independent of how many keys the caller passed.
//
// The one thing JSON_TABLE does NOT need is MySQL's hex round trip. A uuid key is
...
// Left to a default every value comes back as text, and
// comparing that to a UNIQUEIDENTIFIER or a BIGINT puts an implicit conversion
// on the COLUMN side of the predicate — which Oracle will happily do, and
...
func keyRows(keyType string) string {
	// JSON_TABLE, and the alias by juxtaposition: `AS x` on a table is
	// ORA-00933 here.
	return "JSON_TABLE(" + Param(1) + ", '$[*]' COLUMNS (" +
		Ident(parentKeyAlias) + " " + keyType + " PATH '$')) " + Ident(parentAlias)
}
...
// TopNLateral lowers greatest-n-per-group with CROSS APPLY, which is Oracle's
// LATERAL. THE DEFAULT, for the reason PostgreSQL's is: a limited, ordered scan
// per parent key, which the planner stops at N rows.
//
// The cap is FETCH FIRST and goes AFTER the ordering, where SQL Server's TOP
// goes before the column list. That is a text position and not a binding order:
// `:2` is a NAME here, so the generated caller still binds keys then cap
// whichever side of the ordering the cap is written on.
func TopNLateral(table string, cols []string, key, keyType string, live Live) string {
	...
	b.WriteString(" FROM ")
	b.WriteString(keyRows(keyType))
	b.WriteString(" CROSS APPLY (SELECT ")
	writeIdents(&b, cols)
	b.WriteString(" FROM ")
	b.WriteString(Ident(table))
	b.WriteString(" WHERE ")
