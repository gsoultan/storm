...
//
// The shape is standard SQL and carries over whole: Oracle has JOIN, LEFT JOIN,
// table aliases and WITH. Like SQL Server and unlike MySQL it also has
// everything the CTEs CONTAIN — a declared aggregation's GROUPING SETS and CUBE
// both exist here, measured in internal/oraclespike — so a join that
// materialises one is emitted rather than refused.
//
// One spelling differs and it is easy to miss: a table alias is JUXTAPOSITION,
// so `FROM t AS x` is ORA-00933. A column alias still takes AS.
//
...
func JoinSelect(table string, j *schema.Join,
	...
	for _, t := range j.Tables {
		...
		// No AS before a table alias: Oracle's is juxtaposition, and
		// `FROM t AS x` is ORA-00933. Every other target storm has accepts
		// the keyword, which is why this reads like an omission and is not.
...
//
// No NULLS placement: SQL Server has none, and a join's declared ordering is
// what the caller pages by, so dropping an explicit one would page differently
