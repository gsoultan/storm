...
var conflictSpecs = []string{
	" ON CONFLICT (\"post_id\", \"related_id\")",
}
...
// OnConflictPostIDRelatedID upserts on the unique index over (post_id, related_id).
//
...
//	n.OnConflictPostIDRelatedID().DoNothing()  // only on that one index
//
