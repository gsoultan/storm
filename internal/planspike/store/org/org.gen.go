...
func HasChildren() Pred   { return Pred{col: 5, op: opExists} }
func HasNoChildren() Pred { return Pred{col: 5, op: opNotExists} }
func HasUsers() Pred      { return Pred{col: 6, op: opExists} }
...
const existsPrefix = `SELECT 1 FROM "orgs"`
...
var fragTable = [7][28]runtime.Frag{
	...
	{ // relation Children (pseudo-column)
		{}, // opNone
		{},
		{},
		{},
	...
	{ // relation Users (pseudo-column)
		...
		{},
		{},
		{A: "EXISTS (SELECT 1 FROM \"users\" AS \"_storm_e\" WHERE \"_storm_e\".\"org_id\" = \"orgs\".\"id\")"},
		{A: "NOT EXISTS (SELECT 1 FROM \"users\" AS \"_storm_e\" WHERE \"_storm_e\".\"org_id\" = \"orgs\".\"id\")"},
	},
}
...
func batchTopByParentIDWindowSQL(order []Sort) string {
	...
	sql := runtime.SpliceOrder("SELECT \"id\", \"created_at\", \"updated_at\", \"name\", \"parent_id\" FROM (SELECT \"id\", \"created_at\", \"updated_at\", \"name\", \"parent_id\", row_number() OVER (PARTITION BY \"parent_id\"\x00order\x00) AS \"_storm_rn\" FROM \"orgs\" WHERE \"parent_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
...
func batchTopByParentIDLateralSQL(order []Sort) string {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"id\", \"_storm_c\".\"created_at\", \"_storm_c\".\"updated_at\", \"_storm_c\".\"name\", \"_storm_c\".\"parent_id\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"id\", \"created_at\", \"updated_at\", \"name\", \"parent_id\" FROM \"orgs\" WHERE \"parent_id\" = \"_storm_p\".\"_storm_k\"\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
...
const descendSQL = `WITH RECURSIVE "_storm_r" AS (SELECT "id", "created_at", "updated_at", "name", "parent_id", 1 AS "_storm_d", ARRAY["id"] AS "_storm_path" FROM "orgs" WHERE "id" = ANY($1) UNION ALL SELECT "_storm_rc"."id", "_storm_rc"."created_at", "_storm_rc"."updated_at", "_storm_rc"."name", "_storm_rc"."parent_id", "_storm_r"."_storm_d" + 1, "_storm_r"."_storm_path" || "_storm_rc"."id" FROM "orgs" "_storm_rc" JOIN "_storm_r" ON "_storm_rc"."parent_id" = "_storm_r"."id" WHERE "_storm_r"."_storm_d" < $2 AND NOT "_storm_rc"."id" = ANY("_storm_r"."_storm_path")) SELECT "id", "created_at", "updated_at", "name", "parent_id" FROM "_storm_r"`
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"name\", \"parent_id\""
...
// OnConflictName upserts on the unique index over (name).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "orgs", copyCols, src)
}
