...
func (q Query) TagIDEq(v [16]byte) Query        { return q.Where(TagID.Eq(v)) }
func (q Query) TagIDNotEq(v [16]byte) Query     { return q.Where(TagID.NotEq(v)) }
func (q Query) TagIDIn(v ...[16]byte) Query     { return q.Where(TagID.In(v...)) }
func (q Query) TagIDNotIn(v ...[16]byte) Query  { return q.Where(TagID.NotIn(v...)) }

const selectPrefix = `SELECT "post_id", "tag_id" FROM "post_tags"`
const countPrefix = `SELECT count(*) FROM "post_tags"`
const existsPrefix = `SELECT 1 FROM "post_tags"`
...
var orderTable = [nCols][4]string{
	...
	{ // tag_id
		"\"tag_id\"",
		"\"tag_id\" DESC",
		"\"tag_id\" ASC NULLS FIRST",
		"\"tag_id\" DESC NULLS LAST",
	},
}
...
var defaultOrder = [2]runtime.Tok{
	...
	runtime.MakeOrder(runtime.Asc, 1), // tag_id
}
...
const insertSQL = `INSERT INTO "post_tags" ("post_id", "tag_id") VALUES ($1, $2) RETURNING "post_id", "tag_id"`

const updatePrefix = `UPDATE "post_tags" SET `
const deletePrefix = `DELETE FROM "post_tags"`
...
const insPrefix = "INSERT INTO \"post_tags\""
const insReturning = " RETURNING \"post_id\", \"tag_id\""
...
var upsertTails = []func(uint64) string{
	func(mask uint64) string {
		set := make([]string, 0, 0)
		return onConflictPostIDTagID(set)
	},
}

func onConflictPostIDTagID(set []string) string {
	switch len(set) {
	case 0:
		return " ON CONFLICT (\"post_id\", \"tag_id\") DO NOTHING"
	}
	return " ON CONFLICT (\"post_id\", \"tag_id\") DO UPDATE SET " + joinAssign(set)
}

// OnConflictPostIDTagID upserts on the unique key (post_id, tag_id).
func (n *Ins) OnConflictPostIDTagID() *Ins {
	n.conflict = 1
	return n
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "post_tags", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	args = append(args, m.row.TagID)
	return runtime.BatchOp{SQL: st.SQL, Args: args}, true
}

// DeleteOp is a delete as a queueable statement.
func DeleteOp(postID [16]byte, tagID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{postID, tagID}}
}
...
func Delete(ctx context.Context, ex runtime.Executor, postID [16]byte, tagID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{postID, tagID})
	if err != nil {
		return err
	}
	if n == 0 {
