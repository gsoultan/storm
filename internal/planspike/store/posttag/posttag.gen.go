...
const updReturning = " RETURNING \"post_id\", \"tag_id\""
...
func MutateKey(postID [16]byte, tagID [16]byte) Mut {
	return Mut{row: Row{
		PostID: postID,
		TagID:  tagID,
	}}
...
// OnConflictPostIDTagID upserts on the unique index over (post_id, tag_id).
//
...
//	n.OnConflictPostIDTagID().DoNothing()  // only on that one index
//
