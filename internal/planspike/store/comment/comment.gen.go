...
func HasReplies() Pred   { return Pred{col: 7, op: opExists} }
...
const existsPrefix = `SELECT 1 FROM "comments"`
...
var orderTable = [nCols][4]string{
	...
	{ // body
		"\"body\"",
		"\"body\" DESC",
		"\"body\" ASC NULLS FIRST",
		"\"body\" DESC NULLS LAST",
	},
	{ // post_id
		"\"post_id\"",
		"\"post_id\" DESC",
	...
	{ // author_id
		"\"author_id\"",
		"\"author_id\" DESC",
		"\"author_id\" ASC NULLS FIRST",
		"\"author_id\" DESC NULLS LAST",
	},
	{ // parent_id
		"\"parent_id\"",
		"\"parent_id\" DESC",
...
var fragTable = [8][28]runtime.Frag{
	...
	{ // relation Replies (pseudo-column)
		...
		{},
		{},
		{A: "EXISTS (SELECT 1 FROM \"comments\" AS \"_storm_e\" WHERE \"_storm_e\".\"parent_id\" = \"comments\".\"id\")"},
		{A: "NOT EXISTS (SELECT 1 FROM \"comments\" AS \"_storm_e\" WHERE \"_storm_e\".\"parent_id\" = \"comments\".\"id\")"},
	},
}
...
func batchTopByParentIDWindowSQL(order []Sort) string {
	...
	sql := runtime.SpliceOrder("SELECT \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\" FROM (SELECT \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\", row_number() OVER (PARTITION BY \"parent_id\"\x00order\x00) AS \"_storm_rn\" FROM \"comments\" WHERE \"parent_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
...
func batchTopByParentIDLateralSQL(order []Sort) string {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"id\", \"_storm_c\".\"created_at\", \"_storm_c\".\"updated_at\", \"_storm_c\".\"body\", \"_storm_c\".\"post_id\", \"_storm_c\".\"author_id\", \"_storm_c\".\"parent_id\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\" FROM \"comments\" WHERE \"parent_id\" = \"_storm_p\".\"_storm_k\"\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
	return batchTopByParentIDLateralCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

// BatchTopByPostID fetches at most n rows for each id, ordered by order.
//
// ONE round trip regardless of how many ids are passed — the per-parent
// limit is expressed in SQL, not by looping. It delegates to the lowering
// chosen at BUILD time by measurement (bench/RESULTS.md); the strategy is
// never sniffed at run time.
func BatchTopByPostID(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return BatchTopByPostIDLateral(ctx, ex, ids, n, order...)
}

// BatchTopByPostIDWindow runs the window lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByPostIDWindow(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByPostIDRun(ctx, ex, batchTopByPostIDWindowSQL(order), ids, n)
}

// BatchTopByPostIDLateral runs the lateral lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByPostIDLateral(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByPostIDRun(ctx, ex, batchTopByPostIDLateralSQL(order), ids, n)
}

// batchTopByPostIDRun executes one lowered statement.
func batchTopByPostIDRun(ctx context.Context, ex runtime.Executor, sql string, ids [][16]byte, n int64) ([]Row, error) {
	if len(ids) == 0 || n <= 0 {
		return nil, nil
	}
	if sql == "" {
...
var batchTopByPostIDWindowCache = runtime.NewTreeCache()

// batchTopByPostIDWindowSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByPostIDWindowSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByPostIDWindowCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\" FROM (SELECT \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\", row_number() OVER (PARTITION BY \"post_id\"\x00order\x00) AS \"_storm_rn\" FROM \"comments\" WHERE \"post_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
	return batchTopByPostIDWindowCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

var batchTopByPostIDLateralCache = runtime.NewTreeCache()

// batchTopByPostIDLateralSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByPostIDLateralSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByPostIDLateralCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"id\", \"_storm_c\".\"created_at\", \"_storm_c\".\"updated_at\", \"_storm_c\".\"body\", \"_storm_c\".\"post_id\", \"_storm_c\".\"author_id\", \"_storm_c\".\"parent_id\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\" FROM \"comments\" WHERE \"post_id\" = \"_storm_p\".\"_storm_k\"\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
	return batchTopByPostIDLateralCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

// Recursive traversal of comments's self-reference through parent_id.
//
...
var ErrDepth = errors.New(
	"storm: recursive traversal needs a positive depth bound — unbounded recursion over a cycle does not return")
...
const descendSQL = `WITH RECURSIVE "_storm_r" AS (SELECT "id", "created_at", "updated_at", "body", "post_id", "author_id", "parent_id", 1 AS "_storm_d", ARRAY["id"] AS "_storm_path" FROM "comments" WHERE "id" = ANY($1) UNION ALL SELECT "_storm_rc"."id", "_storm_rc"."created_at", "_storm_rc"."updated_at", "_storm_rc"."body", "_storm_rc"."post_id", "_storm_rc"."author_id", "_storm_rc"."parent_id", "_storm_r"."_storm_d" + 1, "_storm_r"."_storm_path" || "_storm_rc"."id" FROM "comments" "_storm_rc" JOIN "_storm_r" ON "_storm_rc"."parent_id" = "_storm_r"."id" WHERE "_storm_r"."_storm_d" < $2 AND NOT "_storm_rc"."id" = ANY("_storm_r"."_storm_path")) SELECT "id", "created_at", "updated_at", "body", "post_id", "author_id", "parent_id" FROM "_storm_r"`
...
const ascendSQL = `WITH RECURSIVE "_storm_r" AS (SELECT "id", "created_at", "updated_at", "body", "post_id", "author_id", "parent_id", 1 AS "_storm_d", ARRAY["id"] AS "_storm_path" FROM "comments" WHERE "id" = ANY($1) UNION ALL SELECT "_storm_rc"."id", "_storm_rc"."created_at", "_storm_rc"."updated_at", "_storm_rc"."body", "_storm_rc"."post_id", "_storm_rc"."author_id", "_storm_rc"."parent_id", "_storm_r"."_storm_d" + 1, "_storm_r"."_storm_path" || "_storm_rc"."id" FROM "comments" "_storm_rc" JOIN "_storm_r" ON "_storm_r"."parent_id" = "_storm_rc"."id" WHERE "_storm_r"."_storm_d" < $2 AND NOT "_storm_rc"."id" = ANY("_storm_r"."_storm_path")) SELECT "id", "created_at", "updated_at", "body", "post_id", "author_id", "parent_id" FROM "_storm_r"`
...
const insertSQL = `INSERT INTO "comments" ("id", "created_at", "updated_at", "body", "post_id", "author_id", "parent_id") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING "id", "created_at", "updated_at", "body", "post_id", "author_id", "parent_id"`

const updatePrefix = `UPDATE "comments" SET `
const deletePrefix = `DELETE FROM "comments"`
...
const (
	...
	dBody      uint64 = 1 << 1
	dPostID    uint64 = 1 << 2
	dAuthorID  uint64 = 1 << 3
	dParentID  uint64 = 1 << 4
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{},                                   // post_id has no server-side form
	{},                                   // author_id has no server-side form
	{},                                   // parent_id has no server-side form
}
...
const (
	...
	iBody      uint64 = 1 << 3
	iPostID    uint64 = 1 << 4
	iAuthorID  uint64 = 1 << 5
	iParentID  uint64 = 1 << 6
)
...
const insPrefix = "INSERT INTO \"comments\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\""
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"body\", \"post_id\", \"author_id\", \"parent_id\""
...
var assignFor = map[string]string{
	...
	"parent_id":  "\"parent_id\" = EXCLUDED.\"parent_id\"",
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "comments", copyCols, src)
}
