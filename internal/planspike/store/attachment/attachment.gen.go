...
func (q Query) FilenameEqLower(v string) Query     { return q.Where(Filename.EqLower(v)) }
...
const existsPrefix = `SELECT 1 FROM "attachments"`
...
func MatchSubject[R any](r Row, post func([16]byte) R, comment func([16]byte) R, user func([16]byte) R) R {
	...
	panic("storm: attachments.Subject has no variant set — the exactly-one CHECK is missing or was dropped")
}
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{},                                   // filename has no server-side form
	{},                                   // post_id has no server-side form
	{},                                   // comment_id has no server-side form
	{},                                   // user_id has no server-side form
}
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"filename\", \"post_id\", \"comment_id\", \"user_id\""
