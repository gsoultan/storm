...
func (q Query) RoleEq(v string) Query           { return q.Where(Role.Eq(v)) }
func (q Query) RoleNotEq(v string) Query        { return q.Where(Role.NotEq(v)) }
func (q Query) RoleGt(v string) Query           { return q.Where(Role.Gt(v)) }
func (q Query) RoleGte(v string) Query          { return q.Where(Role.Gte(v)) }
func (q Query) RoleLt(v string) Query           { return q.Where(Role.Lt(v)) }
func (q Query) RoleLte(v string) Query          { return q.Where(Role.Lte(v)) }
func (q Query) RoleEqLower(v string) Query      { return q.Where(Role.EqLower(v)) }
func (q Query) RoleLike(v string) Query         { return q.Where(Role.Like(v)) }
...
func (q Query) JoinedAtEq(v time.Time) Query    { return q.Where(JoinedAt.Eq(v)) }
func (q Query) JoinedAtNotEq(v time.Time) Query { return q.Where(JoinedAt.NotEq(v)) }
func (q Query) JoinedAtGt(v time.Time) Query    { return q.Where(JoinedAt.Gt(v)) }
func (q Query) JoinedAtGte(v time.Time) Query   { return q.Where(JoinedAt.Gte(v)) }
func (q Query) JoinedAtLt(v time.Time) Query    { return q.Where(JoinedAt.Lt(v)) }
func (q Query) JoinedAtLte(v time.Time) Query   { return q.Where(JoinedAt.Lte(v)) }

const selectPrefix = `SELECT "user_id", "org_id", "role", "joined_at" FROM "memberships"`
const countPrefix = `SELECT count(*) FROM "memberships"`
const existsPrefix = `SELECT 1 FROM "memberships"`
...
var defaultOrder = [2]runtime.Tok{
	runtime.MakeOrder(runtime.Asc, 0), // user_id
	runtime.MakeOrder(runtime.Asc, 1), // org_id
}
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	copy(r.OrgID[:], rv[1])
	r.Role = sl.Str(rv[2])
	r.JoinedAt = runtime.Timestamptz(rv[3])
	return nil
}
...
const insertSQL = `INSERT INTO "memberships" ("user_id", "org_id", "role", "joined_at") VALUES ($1, $2, $3, $4) RETURNING "user_id", "org_id", "role", "joined_at"`

const updatePrefix = `UPDATE "memberships" SET `
const deletePrefix = `DELETE FROM "memberships"`

// Dirty bits. One per updatable column; the set of them is an UPDATE's
// identity, exactly as a token stream is a SELECT's.
const (
	dRole     uint64 = 1 << 0
	dJoinedAt uint64 = 1 << 1
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	{},                                  // role has no server-side form
	{A: "\"joined_at\" = now()", B: ""}, // joined_at = the database's clock
}
...
const insPrefix = "INSERT INTO \"memberships\""
const insReturning = " RETURNING \"user_id\", \"org_id\", \"role\", \"joined_at\""
...
const updReturning = " RETURNING \"user_id\", \"org_id\", \"role\", \"joined_at\""
...
func MutateKey(userID [16]byte, orgID [16]byte) Mut {
	return Mut{row: Row{
		UserID: userID,
		OrgID:  orgID,
	}}
...
func (m *Mut) SetJoinedAt(v time.Time) {
	m.row.JoinedAt = v
	m.dirty |= dJoinedAt
	m.expr &^= dJoinedAt
}

// SetJoinedAtNow assigns the database's clock, not this process's.
//
...
func (n *Ins) SetJoinedAt(v time.Time) {
	n.row.JoinedAt = v
	n.set |= iJoinedAt
}
...
func assignable(i uint8, mask uint64) []string {
	...
	case 0:
		...
		if mask&(1<<3) != 0 {
			set = append(set, "joined_at")
		}
	}
	return set
}

// OnConflictUserIDOrgID upserts on the unique index over (user_id, org_id).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictUserIDOrgID() *Ins {
	n.conflict = 2
	return n
}
...
//	n.OnConflictUserIDOrgID().DoNothing()  // only on that one index
//
...
var assignFor = map[string]string{
	...
	"joined_at": "\"joined_at\" = EXCLUDED.\"joined_at\"",
}
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 3:
			args = append(args, n.row.JoinedAt)
		}
	}
	var out Row
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "memberships", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 1:
			args = append(args, m.row.JoinedAt)
		}
	}
...
func DeleteOp(userID [16]byte, orgID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{userID, orgID}}
}
...
func Delete(ctx context.Context, ex runtime.Executor, userID [16]byte, orgID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{userID, orgID})
	if err != nil {
		return err
	}
	if n == 0 {
