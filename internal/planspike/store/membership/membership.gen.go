...
var upsertTails = []func(uint64) string{
	func(mask uint64) string {
		...
		return onConflictUserIDOrgID(set)
	},
}

func onConflictUserIDOrgID(set []string) string {
	switch len(set) {
	case 0:
		return " ON CONFLICT (\"user_id\", \"org_id\") DO NOTHING"
	}
	return " ON CONFLICT (\"user_id\", \"org_id\") DO UPDATE SET " + joinAssign(set)
}

// OnConflictUserIDOrgID upserts on the unique key (user_id, org_id).
