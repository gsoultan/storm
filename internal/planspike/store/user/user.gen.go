...
type Query struct {
	...
	anyI16                        [3][]int16
	anyI32                        [3][]int32
	anyDec                        [3][]runtime.Decimal
	nar, nas, nai16, nai32, nadec uint8
...
// NullDecimalArrayCol addresses a numeric[] column.
type NullDecimalArrayCol struct{ c uint8 }

func (h NullDecimalArrayCol) Asc() Sort  { return Sort(runtime.MakeOrder(runtime.Asc, uint32(h.c))) }
func (h NullDecimalArrayCol) Desc() Sort { return Sort(runtime.MakeOrder(runtime.Desc, uint32(h.c))) }
func (h NullDecimalArrayCol) AscNullsFirst() Sort {
	return Sort(runtime.MakeOrder(runtime.AscNullsFirst, uint32(h.c)))
}
func (h NullDecimalArrayCol) DescNullsLast() Sort {
	return Sort(runtime.MakeOrder(runtime.DescNullsLast, uint32(h.c)))
}
...
// An empty v is true for every row — every array contains
// the empty one.
func (h NullDecimalArrayCol) Contains(v ...runtime.Decimal) Pred {
	return Pred{col: h.c, op: opArrayContains, anyDec: v}
}

// ContainedBy is `<@`: every element of the column is in v.
func (h NullDecimalArrayCol) ContainedBy(v ...runtime.Decimal) Pred {
	return Pred{col: h.c, op: opArrayContainedBy, anyDec: v}
}

// Overlaps is `&&`: the column and v share an element.
func (h NullDecimalArrayCol) Overlaps(v ...runtime.Decimal) Pred {
	return Pred{col: h.c, op: opArrayOverlaps, anyDec: v}
}
func (h NullDecimalArrayCol) IsNull() Pred    { return Pred{col: h.c, op: opIsNull} }
func (h NullDecimalArrayCol) IsNotNull() Pred { return Pred{col: h.c, op: opIsNotNull} }
...
func (q Query) EmailEqLower(v string) Query               { return q.Where(Email.EqLower(v)) }
...
func (q Query) NameEqLower(v string) Query                { return q.Where(Name.EqLower(v)) }
...
func (q Query) LastIPEqLower(v string) Query              { return q.Where(LastIP.EqLower(v)) }
...
func (q Query) SplitsContains(v ...runtime.Decimal) Query { return q.Where(Splits.Contains(v...)) }
func (q Query) SplitsContainedBy(v ...runtime.Decimal) Query {
	return q.Where(Splits.ContainedBy(v...))
}
func (q Query) SplitsOverlaps(v ...runtime.Decimal) Query { return q.Where(Splits.Overlaps(v...)) }
func (q Query) SplitsIsNull() Query                       { return q.Where(Splits.IsNull()) }
func (q Query) SplitsIsNotNull() Query                    { return q.Where(Splits.IsNotNull()) }
...
var orderTable = [nCols][4]string{
	...
	{ // splits
		"\"splits\"",
		"\"splits\" DESC",
		"\"splits\" ASC NULLS FIRST",
		"\"splits\" DESC NULLS LAST",
	},
	{ // org_id
		"\"org_id\"",
		"\"org_id\" DESC",
...
var fragTable = [18][28]runtime.Frag{
	{ // id
		{}, // opNone
		{A: "\"id\" = $", B: ""},
		{A: "\"id\" <> $", B: ""},
	...
	{ // relation Posts (pseudo-column)
		...
		{},
		{},
		{A: "EXISTS (SELECT 1 FROM \"posts\" AS \"_storm_e\" WHERE \"_storm_e\".\"author_id\" = \"users\".\"id\")"},
		{A: "NOT EXISTS (SELECT 1 FROM \"posts\" AS \"_storm_e\" WHERE \"_storm_e\".\"author_id\" = \"users\".\"id\")"},
	},
}
...
func putBinder(b *binder) {
	...
	for i := range b.anyI16 {
		b.anyI16[i] = nil
	}
	for i := range b.anyI32 {
		b.anyI32[i] = nil
	}
	for i := range b.anyDec {
		b.anyDec[i] = nil
	}
	binders.Put(b)
}
	...
	for i := uint8(0); i < q.nt; i++ {
		...
		case opIn, opNotIn, opArrayContains, opArrayContainedBy, opArrayOverlaps, opHasAnyKey, opHasAllKeys:
			...
			case 9:
				...
				nas++
			case 10:
				b.anyI16[nai16] = q.anyI16[nai16]
				v = append(v, &b.anyI16[nai16])
				nai16++
			case 11:
				b.anyStr[nas] = q.anyStr[nas]
				v = append(v, &b.anyStr[nas])
				nas++
			case 15:
				b.anyDec[nadec] = q.anyDec[nadec]
				v = append(v, &b.anyDec[nadec])
				nadec++
			case 16:
				b.anyRaw[nar] = q.anyRaw[nar]
				v = append(v, &b.anyRaw[nar])
...
// FinRow is the "Fin" projection: the same read, 3 column(s) instead of
// the whole row. Narrower tuples, no TOAST fetch for what nobody asked,
// and an index-only scan becomes POSSIBLE — the full-row read forecloses
// it by construction.
type FinRow struct {
...
const finPrefix = `SELECT "email", "balance", "age" FROM "users"`

var (
	finCache       = runtime.NewTreeCache()
	finOffsetCache = runtime.NewTreeCache()
)

func finStmtFor(toks []runtime.Tok, withOffset bool) *runtime.Stmt {
	c, suffix := finCache, limitSuffix
	if withOffset {
		c, suffix = finOffsetCache, limitOffsetSuffix
	}
	if st := c.Get(toks); st != nil {
		return st
	}
	return c.Put(toks, runtime.SpliceTree(finPrefix, toks, lowering, suffix))
}

func scanFin(rv [][]byte, r *FinRow, sl *runtime.Slab) error {
	var decErr error
	r.Email = sl.Str(rv[0])
	r.Balance, decErr = runtime.NumericErr(rv[1])
	if decErr != nil {
		return decErr
	}
	r.Age = runtime.Nullable(rv[2], runtime.Int2)
	return nil
}

// AllFin runs the query projected to FinRow. Predicates, ordering, limit,
// offset and keyset all apply exactly as on All — a projection changes
// what a row CARRIES, never which rows qualify. (After takes the full Row;
// populate its ordering columns and the cursor works unchanged.)
func (q Query) AllFin(ctx context.Context, ex runtime.Executor) ([]FinRow, error) {
	var sl runtime.Slab
	return q.AllFinInto(ctx, ex, nil, &sl)
}

// AllFinInto lets the caller own the output slice and the arena, exactly
// as AllInto does — a projection's terminals mirror the full read's, so a
// hot loop reuses both and a benchmark compares like with like.
func (q Query) AllFinInto(ctx context.Context, ex runtime.Executor, dst []FinRow, sl *runtime.Slab) ([]FinRow, error) {
	if err := q.Err(); err != nil {
		return dst, err
	}
	var buf [21]runtime.Tok
	st := finStmtFor(q.stream(&buf), q.offset > 0)
	if st.Err != nil {
		return dst, st.Err
	}
	sl.Reserve(st.SlabHint())
	...
	defer rows.Close()
	for rows.Next() {
		dst = append(dst, FinRow{})
		if err := scanFin(rows.RawValues(), &dst[len(dst)-1], sl); err != nil {
			return dst, err
		}
	}
	st.ObserveSlab(sl.Size())
...
// OneFin is AllFin stopped at one row.
func (q Query) OneFin(ctx context.Context, ex runtime.Executor) (FinRow, bool, error) {
	out, err := q.Limit(1).AllFin(ctx, ex)
	if err != nil || len(out) == 0 {
		return FinRow{}, false, err
	}
	return out[0], true, nil
}
...
var errWithOrgLocked = errors.New(
...
var errMaybeOrgLocked = errors.New(
	"storm: a declared join cannot be row-locked — the server refuses a row lock on the nullable side of an outer join, and which side a lock would take is not something the call site says; lock the base read instead")
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{A: "\"deleted_at\" = now()", B: ""}, // deleted_at = the database's clock
	{},                                   // email has no server-side form
	{},                                   // name has no server-side form
	{},                                   // status has no server-side form
	{},                                   // prefs has no server-side form
	{},                                   // scopes has no server-side form
	{A: "\"age\" = \"age\" + 1", B: ""},  // age = its own value plus one
	{},                                   // last_ip has no server-side form
	{},                                   // active has no server-side form
	{},                                   // balance has no server-side form
	{},                                   // credit has no server-side form
	{},                                   // splits has no server-side form
	{},                                   // org_id has no server-side form
}
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"version\", \"deleted_at\", \"email\", \"name\", \"status\", \"prefs\", \"scopes\", \"age\", \"last_ip\", \"active\", \"balance\", \"credit\", \"splits\", \"org_id\""
...
// There is no MutateKey on this table. The version column makes every
// update an optimistic lock, and a lock needs the version that was
// READ — an address alone would carry zero and match nothing, which
// reads as ErrStaleWrite and tells the caller the wrong story.
...
// SetDeletedAtNow assigns the database's clock, not this process's.
//
// Binding time.Now() instead would record when THIS server
// thought it was. Servers skew, so rows written seconds apart
// can land out of order, and an event stamped by a fast clock
// reads as preceding the thing that caused it.
func (m *Mut) SetDeletedAtNow() {
	m.expr |= dDeletedAt
	m.dirty &^= dDeletedAt
}
...
// IncAge adds one to the column's OWN value, in the database.
//
...
func (m *Mut) SetSplits(v []runtime.Decimal) {
	m.row.Splits = v
	m.dirty |= dSplits
	m.expr &^= dSplits
}
...
func (n *Ins) SetSplits(v []runtime.Decimal) {
	n.row.Splits = v
	n.set |= iSplits
}
...
// OnConflictLowerEmail upserts on the unique index over (lower(email)).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
