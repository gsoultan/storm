...
// TextCol addresses a varchar(32) column.
...
func (h TSVectorCol) Asc() Sort  { return Sort(runtime.MakeOrder(runtime.Asc, uint32(h.c))) }
func (h TSVectorCol) Desc() Sort { return Sort(runtime.MakeOrder(runtime.Desc, uint32(h.c))) }
func (h TSVectorCol) AscNullsFirst() Sort {
	return Sort(runtime.MakeOrder(runtime.AscNullsFirst, uint32(h.c)))
}
func (h TSVectorCol) DescNullsLast() Sort {
	return Sort(runtime.MakeOrder(runtime.DescNullsLast, uint32(h.c)))
}

func (h TSVectorCol) Matches(v string) Pred   { return Pred{col: h.c, op: opMatches, str: v} }
func (h TSVectorCol) WebSearch(v string) Pred { return Pred{col: h.c, op: opWebSearch, str: v} }
...
func (q Query) SkuEqLower(v string) Query             { return q.Where(Sku.EqLower(v)) }
...
var orderTable = [nCols][4]string{
	...
	{ // search
		"\"search\"",
		"\"search\" DESC",
		"\"search\" ASC NULLS FIRST",
		"\"search\" DESC NULLS LAST",
	},
}
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.Sku = sl.Str(rv[3])
	r.Name = sl.Str(rv[4])
	r.Price, decErr = runtime.NumericErr(rv[5])
	if decErr != nil {
		return decErr
	}
...
// CardRow is the "Card" projection: the same read, 3 column(s) instead of
...
const cardPrefix = `SELECT "sku", "name", "price" FROM "products"`

var (
	cardCache       = runtime.NewTreeCache()
	cardOffsetCache = runtime.NewTreeCache()
)

func cardStmtFor(toks []runtime.Tok, withOffset bool) *runtime.Stmt {
	c, suffix := cardCache, limitSuffix
	if withOffset {
		c, suffix = cardOffsetCache, limitOffsetSuffix
	}
	if st := c.Get(toks); st != nil {
		return st
	}
	return c.Put(toks, runtime.SpliceTree(cardPrefix, toks, lowering, suffix))
}

func scanCard(rv [][]byte, r *CardRow, sl *runtime.Slab) error {
	var decErr error
	r.Sku = sl.Str(rv[0])
	r.Name = sl.Str(rv[1])
	r.Price, decErr = runtime.NumericErr(rv[2])
	if decErr != nil {
		return decErr
	}
	return nil
...
// AllCard runs the query projected to CardRow. Predicates, ordering, limit,
...
func (q Query) AllCard(ctx context.Context, ex runtime.Executor) ([]CardRow, error) {
	var sl runtime.Slab
	return q.AllCardInto(ctx, ex, nil, &sl)
}

// AllCardInto lets the caller own the output slice and the arena, exactly
...
func (q Query) AllCardInto(ctx context.Context, ex runtime.Executor, dst []CardRow, sl *runtime.Slab) ([]CardRow, error) {
	if err := q.Err(); err != nil {
		return dst, err
	}
	var buf [21]runtime.Tok
	st := cardStmtFor(q.stream(&buf), q.offset > 0)
	if st.Err != nil {
		return dst, st.Err
	}
	sl.Reserve(st.SlabHint())
	...
	defer rows.Close()
	for rows.Next() {
		dst = append(dst, CardRow{})
		if err := scanCard(rows.RawValues(), &dst[len(dst)-1], sl); err != nil {
			return dst, err
		}
	}
	st.ObserveSlab(sl.Size())
...
// OneCard is AllCard stopped at one row.
func (q Query) OneCard(ctx context.Context, ex runtime.Executor) (CardRow, bool, error) {
	out, err := q.Limit(1).AllCard(ctx, ex)
	if err != nil || len(out) == 0 {
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"sku\", \"name\", \"price\", \"active\", \"tags\", \"attrs\", \"search\""
...
func (m *Mut) SetPrice(v runtime.Decimal) {
	m.row.Price = v
	m.dirty |= dPrice
	m.expr &^= dPrice
}
...
func (n *Ins) SetPrice(v runtime.Decimal) {
	n.row.Price = v
	n.set |= iPrice
}
...
// OnConflictSku upserts on the unique index over (sku).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictSku() *Ins {
	n.conflict = 4
	return n
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "products", copyCols, src)
}
