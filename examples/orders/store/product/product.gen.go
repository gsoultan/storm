...
func onConflictSku(set []string) string {
	switch len(set) {
	case 0:
		return " ON CONFLICT (\"sku\") DO NOTHING"
	}
	return " ON CONFLICT (\"sku\") DO UPDATE SET " + joinAssign(set)
}

// OnConflictSku upserts on the unique key (sku).
