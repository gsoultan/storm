...
type Query struct {
	...
	raws                 [4][16]byte
	tims                 [4]time.Time
	rngs                 [4]runtime.TstzRange
	ns, nn, nr, ntm, nrg uint8
...
func (q *Query) cursor(col uint32, r Row) {
	...
	case 4:
		...
		q.ns++
	case 5:
		if int(q.nrg) >= len(q.rngs) {
			q.over = true
			return
		}
		q.rngs[q.nrg] = r.During
		q.nrg++
	}
}
...
// TstzRangeCol addresses a tstzrange column.
type TstzRangeCol struct{ c uint8 }

func (h TstzRangeCol) Asc() Sort  { return Sort(runtime.MakeOrder(runtime.Asc, uint32(h.c))) }
func (h TstzRangeCol) Desc() Sort { return Sort(runtime.MakeOrder(runtime.Desc, uint32(h.c))) }
func (h TstzRangeCol) AscNullsFirst() Sort {
	return Sort(runtime.MakeOrder(runtime.AscNullsFirst, uint32(h.c)))
}
func (h TstzRangeCol) DescNullsLast() Sort {
	return Sort(runtime.MakeOrder(runtime.DescNullsLast, uint32(h.c)))
}

func (h TstzRangeCol) Eq(v runtime.TstzRange) Pred    { return Pred{col: h.c, op: opEq, rng: v} }
func (h TstzRangeCol) NotEq(v runtime.TstzRange) Pred { return Pred{col: h.c, op: opNotEq, rng: v} }
func (h TstzRangeCol) Overlaps(v runtime.TstzRange) Pred {
	return Pred{col: h.c, op: opOverlaps, rng: v}
}
func (h TstzRangeCol) ContainsRange(v runtime.TstzRange) Pred {
	return Pred{col: h.c, op: opContainsRange, rng: v}
}
func (h TstzRangeCol) ContainedBy(v runtime.TstzRange) Pred {
	return Pred{col: h.c, op: opContainedBy, rng: v}
}
...
func (q Query) RoomEq(v int32) Query                     { return q.Where(Room.Eq(v)) }
func (q Query) RoomNotEq(v int32) Query                  { return q.Where(Room.NotEq(v)) }
func (q Query) RoomGt(v int32) Query                     { return q.Where(Room.Gt(v)) }
func (q Query) RoomGte(v int32) Query                    { return q.Where(Room.Gte(v)) }
func (q Query) RoomLt(v int32) Query                     { return q.Where(Room.Lt(v)) }
func (q Query) RoomLte(v int32) Query                    { return q.Where(Room.Lte(v)) }
func (q Query) RoomIn(v ...int32) Query                  { return q.Where(Room.In(v...)) }
func (q Query) RoomNotIn(v ...int32) Query               { return q.Where(Room.NotIn(v...)) }
func (q Query) GuestEq(v string) Query                   { return q.Where(Guest.Eq(v)) }
func (q Query) GuestNotEq(v string) Query                { return q.Where(Guest.NotEq(v)) }
func (q Query) GuestGt(v string) Query                   { return q.Where(Guest.Gt(v)) }
func (q Query) GuestGte(v string) Query                  { return q.Where(Guest.Gte(v)) }
func (q Query) GuestLt(v string) Query                   { return q.Where(Guest.Lt(v)) }
func (q Query) GuestLte(v string) Query                  { return q.Where(Guest.Lte(v)) }
func (q Query) GuestEqLower(v string) Query              { return q.Where(Guest.EqLower(v)) }
func (q Query) GuestLike(v string) Query                 { return q.Where(Guest.Like(v)) }
func (q Query) GuestILike(v string) Query                { return q.Where(Guest.ILike(v)) }
func (q Query) GuestIn(v ...string) Query                { return q.Where(Guest.In(v...)) }
func (q Query) GuestNotIn(v ...string) Query             { return q.Where(Guest.NotIn(v...)) }
func (q Query) DuringEq(v runtime.TstzRange) Query       { return q.Where(During.Eq(v)) }
func (q Query) DuringNotEq(v runtime.TstzRange) Query    { return q.Where(During.NotEq(v)) }
func (q Query) DuringOverlaps(v runtime.TstzRange) Query { return q.Where(During.Overlaps(v)) }
func (q Query) DuringContainsRange(v runtime.TstzRange) Query {
	return q.Where(During.ContainsRange(v))
}
func (q Query) DuringContainedBy(v runtime.TstzRange) Query { return q.Where(During.ContainedBy(v)) }

const selectPrefix = `SELECT "id", "created_at", "updated_at", "room", "guest", "during" FROM "bookings"`
const countPrefix = `SELECT count(*) FROM "bookings"`
const existsPrefix = `SELECT 1 FROM "bookings"`
...
var orderTable = [nCols][4]string{
	...
	{ // room
		"\"room\"",
		"\"room\" DESC",
		"\"room\" ASC NULLS FIRST",
		"\"room\" DESC NULLS LAST",
	},
	{ // guest
		"\"guest\"",
		"\"guest\" DESC",
		"\"guest\" ASC NULLS FIRST",
		"\"guest\" DESC NULLS LAST",
	},
	{ // during
		"\"during\"",
		"\"during\" DESC",
		"\"during\" ASC NULLS FIRST",
		"\"during\" DESC NULLS LAST",
	},
}
...
var fragTable = [6][28]runtime.Frag{
	{ // id
		{}, // opNone
		{A: "\"id\" = $", B: ""},
		{A: "\"id\" <> $", B: ""},
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.Room = runtime.Int4(rv[3])
	r.Guest = sl.Str(rv[4])
	r.During, decErr = runtime.TstzRangeErr(rv[5])
	if decErr != nil {
		return decErr
	}
	return decErr
...
func (q Query) bindPreds(b *binder) []any {
	v := b.vals[:0]
	var ns, nn, nr, ntm, nrg, nar, nas, nai32 uint8
	for i := uint8(0); i < q.nt; i++ {
		...
		case 4:
			...
			ns++
		case 5:
			b.rngs[nrg] = q.rngs[nrg]
			v = append(v, &b.rngs[nrg])
			nrg++
		}
	}
	b.vals = v
...
const insertSQL = `INSERT INTO "bookings" ("id", "created_at", "updated_at", "room", "guest", "during") VALUES ($1, $2, $3, $4, $5, $6) RETURNING "id", "created_at", "updated_at", "room", "guest", "during"`

const updatePrefix = `UPDATE "bookings" SET `
const deletePrefix = `DELETE FROM "bookings"`
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{A: "\"room\" = \"room\" + 1", B: ""}, // room = its own value plus one
	{},                                    // guest has no server-side form
	{},                                    // during has no server-side form
}
...
const insPrefix = "INSERT INTO \"bookings\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"room\", \"guest\", \"during\""
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"room\", \"guest\", \"during\""
...
func (m *Mut) SetRoom(v int32) {
	m.row.Room = v
	m.dirty |= dRoom
	m.expr &^= dRoom
}

// IncRoom adds one to the column's OWN value, in the database.
//
...
func (m *Mut) SetGuest(v string) {
	m.row.Guest = v
	m.dirty |= dGuest
	m.expr &^= dGuest
}

func (m *Mut) SetDuring(v runtime.TstzRange) {
	m.row.During = v
	m.dirty |= dDuring
	m.expr &^= dDuring
}
...
func (n *Ins) SetRoom(v int32) {
	n.row.Room = v
	n.set |= iRoom
}

func (n *Ins) SetGuest(v string) {
	n.row.Guest = v
	n.set |= iGuest
}

func (n *Ins) SetDuring(v runtime.TstzRange) {
	n.row.During = v
	n.set |= iDuring
}
...
func assignable(i uint8, mask uint64) []string {
	...
	case 0:
		...
		if mask&(1<<4) != 0 {
			set = append(set, "guest")
		}
		if mask&(1<<5) != 0 {
			set = append(set, "during")
		}
	}
	return set
}
...
var assignFor = map[string]string{
	...
	"room":       "\"room\" = EXCLUDED.\"room\"",
	"guest":      "\"guest\" = EXCLUDED.\"guest\"",
	"during":     "\"during\" = EXCLUDED.\"during\"",
}
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 3:
			args = append(args, n.row.Room)
		case 4:
			args = append(args, n.row.Guest)
		case 5:
			args = append(args, n.row.During)
		}
	}
	var out Row
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	...
	args = append(args, r.Room)
	args = append(args, r.Guest)
	args = append(args, r.During)
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "bookings", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 1:
			args = append(args, m.row.Room)
		case 2:
			args = append(args, m.row.Guest)
		case 3:
			args = append(args, m.row.During)
		}
	}
