...
func HasArticles() Pred   { return Pred{col: 5, op: opExists} }
func HasNoArticles() Pred { return Pred{col: 5, op: opNotExists} }
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "name", "email" FROM "authors"`
const countPrefix = `SELECT count(*) FROM "authors"`
const existsPrefix = `SELECT 1 FROM "authors"`
...
var fragTable = [6][28]runtime.Frag{
	...
	{ // relation Articles (pseudo-column)
		{}, // opNone
		{},
		{},
		{},
		...
		{},
		{},
		{A: "EXISTS (SELECT 1 FROM \"articles\" AS \"_storm_e\" WHERE \"_storm_e\".\"author_id\" = \"authors\".\"id\")"},
		{A: "NOT EXISTS (SELECT 1 FROM \"articles\" AS \"_storm_e\" WHERE \"_storm_e\".\"author_id\" = \"authors\".\"id\")"},
	},
}
...
// CardRow is the "Card" projection: the same read, 2 column(s) instead of
...
const cardPrefix = `SELECT "name", "email" FROM "authors"`
...
const insertSQL = `INSERT INTO "authors" ("id", "created_at", "updated_at", "name", "email") VALUES ($1, $2, $3, $4, $5) RETURNING "id", "created_at", "updated_at", "name", "email"`

const updatePrefix = `UPDATE "authors" SET `
const deletePrefix = `DELETE FROM "authors"`
...
const insPrefix = "INSERT INTO \"authors\""
const insReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"name\", \"email\""
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"name\", \"email\""
...
// OnConflictEmail upserts on the unique index over (email).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictEmail() *Ins {
	n.conflict = 4
	return n
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "authors", copyCols, src)
}
