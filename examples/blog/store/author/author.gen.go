...
func onConflictEmail(set []string) string {
	switch len(set) {
	case 0:
		return " ON CONFLICT (\"email\") DO NOTHING"
	}
	return " ON CONFLICT (\"email\") DO UPDATE SET " + joinAssign(set)
}

// OnConflictEmail upserts on the unique key (email).
