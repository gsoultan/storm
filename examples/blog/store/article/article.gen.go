...
// TextCol addresses a varchar(300) column.
...
func (q Query) TitleEq(v string) Query             { return q.Where(Title.Eq(v)) }
func (q Query) TitleNotEq(v string) Query          { return q.Where(Title.NotEq(v)) }
func (q Query) TitleGt(v string) Query             { return q.Where(Title.Gt(v)) }
...
func (q Query) PublishedAtGte(v time.Time) Query   { return q.Where(PublishedAt.Gte(v)) }
func (q Query) PublishedAtLt(v time.Time) Query    { return q.Where(PublishedAt.Lt(v)) }
...
const selectPrefix = `SELECT "id", "created_at", "updated_at", "title", "body", "published_at", "author_id" FROM "articles"`
const countPrefix = `SELECT count(*) FROM "articles"`
const existsPrefix = `SELECT 1 FROM "articles"`
...
// BatchTopByAuthorID fetches at most n rows for each id, ordered by order.
//
...
func BatchTopByAuthorID(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return BatchTopByAuthorIDLateral(ctx, ex, ids, n, order...)
}

// BatchTopByAuthorIDWindow runs the window lowering.
//
...
// involve writing SQL.
func BatchTopByAuthorIDWindow(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByAuthorIDRun(ctx, ex, batchTopByAuthorIDWindowSQL(order), ids, n)
}

// BatchTopByAuthorIDLateral runs the lateral lowering.
//
// Both forms are exported so a benchmark can compare them on identical
// generated code rather than on two hand-written approximations, and so
// a caller whose data defeats the default has a way out that does not
// involve writing SQL.
func BatchTopByAuthorIDLateral(ctx context.Context, ex runtime.Executor, ids [][16]byte, n int64, order ...Sort) ([]Row, error) {
	return batchTopByAuthorIDRun(ctx, ex, batchTopByAuthorIDLateralSQL(order), ids, n)
}

// batchTopByAuthorIDRun executes one lowered statement.
func batchTopByAuthorIDRun(ctx context.Context, ex runtime.Executor, sql string, ids [][16]byte, n int64) ([]Row, error) {
	if len(ids) == 0 || n <= 0 {
		return nil, nil
	}
	if sql == "" {
...
var batchTopByAuthorIDWindowCache = runtime.NewTreeCache()

// batchTopByAuthorIDWindowSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByAuthorIDWindowSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	...
	if st := batchTopByAuthorIDWindowCache.Get(toks); st != nil {
		return st.SQL
	}
	...
	sql := runtime.SpliceOrder("SELECT \"id\", \"created_at\", \"updated_at\", \"title\", \"body\", \"published_at\", \"author_id\" FROM (SELECT \"id\", \"created_at\", \"updated_at\", \"title\", \"body\", \"published_at\", \"author_id\", row_number() OVER (PARTITION BY \"author_id\"\x00order\x00) AS \"_storm_rn\" FROM \"articles\" WHERE \"author_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
	return batchTopByAuthorIDWindowCache.Put(toks, &runtime.Stmt{SQL: sql, NArg: 2}).SQL
}

var batchTopByAuthorIDLateralCache = runtime.NewTreeCache()

// batchTopByAuthorIDLateralSQL compiles one ordering, once. The ordering is the key, exactly
// as it is for an ordinary read; an empty ordering yields "", which the
// runner turns into errNoTopOrder.
func batchTopByAuthorIDLateralSQL(order []Sort) string {
	if len(order) == 0 {
		return ""
	}
	toks := make([]runtime.Tok, len(order))
	...
	if st := batchTopByAuthorIDLateralCache.Get(toks); st != nil {
		return st.SQL
	}
	terms := make([]string, len(toks))
	for i, t := range toks {
...
const insertSQL = `INSERT INTO "articles" ("id", "created_at", "updated_at", "title", "body", "published_at", "author_id") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING "id", "created_at", "updated_at", "title", "body", "published_at", "author_id"`

const updatePrefix = `UPDATE "articles" SET `
const deletePrefix = `DELETE FROM "articles"`
...
const insPrefix = "INSERT INTO \"articles\""
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"title\", \"body\", \"published_at\", \"author_id\""
...
func (n *Ins) SetPublishedAt(v time.Time) {
	n.row.PublishedAt = runtime.Null[time.Time]{V: v, Valid: true}
	n.set |= iPublishedAt
}

// SetPublishedAtNull writes SQL NULL explicitly, which is not the same as
// leaving the column unset and taking its default.
func (n *Ins) SetPublishedAtNull() {
	n.row.PublishedAt = runtime.Null[time.Time]{}
	n.set |= iPublishedAt
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "articles", copyCols, src)
}
