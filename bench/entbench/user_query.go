...
func (_q *UserQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("entbench: check existence: %w", err)
	default:
		return true, nil
	}
}
...
//
//	client.User.Query().
//		GroupBy(user.FieldOrgID).
//		Aggregate(entbench.Count()).
//		Scan(ctx, &v)
...
func (_q *UserQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range _q.inters {
		if inter == nil {
			return fmt.Errorf("entbench: uninitialized interceptor (forgotten import entbench/runtime?)")
		}
	...
	for _, f := range _q.ctx.Fields {
		if !user.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("entbench: invalid field %q for query", f)}
		}
	}
	if _q.path != nil {
