...
func (q Query) AgeIn(v ...int32) Query           { return q.Where(Age.In(v...)) }
func (q Query) AgeNotIn(v ...int32) Query        { return q.Where(Age.NotIn(v...)) }
...
func batchTopByOrgIDWindowSQL(order []Sort) string {
	...
	sql := runtime.SpliceOrder("SELECT \"id\", \"org_id\", \"email\", \"name\", \"age\", \"status\", \"created_at\", \"updated_at\" FROM (SELECT \"id\", \"org_id\", \"email\", \"name\", \"age\", \"status\", \"created_at\", \"updated_at\", row_number() OVER (PARTITION BY \"org_id\"\x00order\x00) AS \"_storm_rn\" FROM \"users\" WHERE \"org_id\" = ANY($1)) \"_storm_t\" WHERE \"_storm_rn\" <= $2", terms, " ORDER BY ", ", ")
...
func batchTopByOrgIDLateralSQL(order []Sort) string {
	...
	sql := runtime.SpliceOrder("SELECT \"_storm_c\".\"id\", \"_storm_c\".\"org_id\", \"_storm_c\".\"email\", \"_storm_c\".\"name\", \"_storm_c\".\"age\", \"_storm_c\".\"status\", \"_storm_c\".\"created_at\", \"_storm_c\".\"updated_at\" FROM unnest($1::uuid[]) AS \"_storm_p\"(\"_storm_k\") CROSS JOIN LATERAL (SELECT \"id\", \"org_id\", \"email\", \"name\", \"age\", \"status\", \"created_at\", \"updated_at\" FROM \"users\" WHERE \"org_id\" = \"_storm_p\".\"_storm_k\"\x00order\x00 LIMIT $2) \"_storm_c\"", terms, " ORDER BY ", ", ")
...
const updReturning = " RETURNING \"id\", \"org_id\", \"email\", \"name\", \"age\", \"status\", \"created_at\", \"updated_at\""
