# raorm — engineering conventions & developer profile roster
...
Model-first: one Go schema, generated API, generated migrations raorm never
applies.
...
`schema/` · `query/` · `compile/` · `codegen/` · `runtime/` · `cmd/raorm/`.
...
- **raorm emits migrations but never applies DDL** (ADR-0001). No runtime code
...
| :--- | :--- | :--- | :--- |
| **compiler** | `query/`, `compile/`, `codegen/`, shape enumeration, fragment lowering | an identifier reaching SQL text from a runtime value; a builder node that allocates per call; non-deterministic generated output; a lowering rule without a golden test | golden-file suite; `raorm verify` clean; fuzz corpus green |
...
| **dba** | `model/`, `schema/`, `migrate/`, introspection, EXPLAIN gates, `bench/` fixtures | raorm *applying* DDL; a destructive migration step without `--allow-destructive`; a query added to a hot path without `EXPLAIN (ANALYZE, BUFFERS)`; an N+1 shipped; a relation load with unbounded round trips | `raorm explain` in CI; round-trip counting decorator; model → DDL → introspect round-trip diff empty; `verify --pending` green |
