...
type PackageOptions struct {
	...
	// Import is the module path of storm, e.g. "github.com/gsoultan/storm".
	Import string
	...
	// Shapes are the model types this schema was built from. When set, and
	// when Package is set, a compile-time staleness check is emitted: the
	// generated tree stops compiling if a model's fields change without a
	// regenerate, instead of silently missing the column.
	Shapes []ModelShape
...
func Package(s *schema.Schema, o PackageOptions) (map[string][]byte, error) {
	...
	if o.Package != "" {
		...
		if len(o.Shapes) > 0 {
			shape, err := shapeFile(o.Shapes, o.Package)
			if err != nil {
				return nil, err
			}
			out["shape.gen.go"] = shape
		}
...
func contextFile(s *schema.Schema, o PackageOptions, names []string) ([]byte, error) {
	...
	if err := checkPlanNameCollisions(plans, named); err != nil {
		return nil, err
	}

	var body gen
	body.dec = decodersFor(o.Dialect, o.Import)
	...
	for _, n := range names {
		...
		variantPkg[t.Name] = pkg
	}
	// Every package an arc loader names has to be imported. Collected as the
	// loaders are emitted, for the same reason the plan packages are: a fixed
	// list imports something with no arc and Go rejects an unused import.
	arcPkgs := map[string]bool{}
	arcOwners := 0
	for _, n := range names {
		t := s.Table(n)
		if t == nil || len(t.Arcs) == 0 {
			continue
		}
		owner, err := PackageName(t.GoName, t.Name)
		if err != nil {
			return nil, err
		}
		for _, arc := range t.Arcs {
			...
			for _, v := range arc.Variants {
				if variantPkg[v.Table] == "" {
					missing = v.Table
				}
			}
			if missing != "" {
				return nil, fmt.Errorf(
					"codegen: table %s arc %s has variant table %s outside this context — "+
						"an arc loader must be able to name every variant",
					t.Name, arc.Field, missing)
			}
			body.arcLoader(owner, t, arc, variantPkg)
			arcPkgs[owner] = true
			for _, v := range arc.Variants {
				arcPkgs[variantPkg[v.Table]] = true
			}
			arcOwners++
		}
	}
	...
	var g gen
	g.dec = decodersFor(o.Dialect, o.Import)
...
// checkPlanNameCollisions catches a DECLARED plan whose generated type name is
// already taken by the automatic per-relation tier.
//
// Both tiers land in one package and both derive a name from the parent: the
// relation tier emits Order + "With" + Lines, and a plan declared as
// p.Named("WithLines") emits Order + "WithLines". They are the same identifier,
// so the generated context package does not compile — and the error surfaces
// in a file the developer did not write, naming a type they never typed.
//
// The declaration is also redundant when it collides: the relation tier already
// provides exactly that plan, for free, with a per-child Order and Top the
// declared one does not have. So the fix is to delete it, and the message says
// so.
func checkPlanNameCollisions(plans []relPlan, named []namedPlan) error {
	byName := make(map[string]*relPlan, len(plans))
	for i := range plans {
		byName[plans[i].Name] = &plans[i]
	}
	for _, np := range named {
		rp, clash := byName[np.Name]
		if !clash {
			continue
		}
		return fmt.Errorf(
			"codegen: table %s declares a plan named %q, but %s is already generated "+
				"for the %s relation\n"+
				"       every relation gets a plan of its own automatically — delete the "+
				"declaration and call %s(), or name the declared plan something else",
			np.parent.Name, strings.TrimPrefix(np.Name, exportName(np.parent.GoName)),
			np.Name, rp.rel.Field, np.Name)
	}
	return nil
}

// planPackages is every table package the plan layer names, sorted.
func planPackages(plans []relPlan, named []namedPlan, extra map[string]bool) []string {
