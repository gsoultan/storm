...
func (g *gen) projection(name string, columns []string) {
	...
	g.p("func (q Query) All%s(ctx context.Context, ex "+g.execType()+") ([]%sRow, error) {", name, name)
	...
	g.p("func (q Query) All%sInto(ctx context.Context, ex "+g.execType()+", dst []%sRow, sl *runtime.Slab) ([]%sRow, error) {", name, name, name)
	...
	g.p("func (q Query) One%s(ctx context.Context, ex "+g.execType()+") (%sRow, bool, error) {", name, name)
