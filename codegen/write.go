...
func (g *gen) insType(ins []colInfo) {
	...
	g.p("func (n *Ins) Insert(ctx context.Context, ex "+g.execType()+") (Row, error) {")
	g.p("\tif n.set == 0 {")
...
func (g *gen) insertFn(ins []colInfo) {
	...
	g.p("func Insert(ctx context.Context, ex "+g.execType()+", r *Row) error {")
...
func (g *gen) updateFn(upd, pk []colInfo) {
	...
	g.p("func (m *Mut) Update(ctx context.Context, ex "+g.execType()+") error {")
	...
	if refresh {
		...
		g.p("func (m *Mut) updateReturning(ctx context.Context, ex "+g.execType()+", st *runtime.Stmt, args []any) error {")
		g.p("\trows, err := ex.Query(ctx, st.SQL, args)")
		g.p("\tif err != nil {")
		g.p("\t\treturn err")
		g.p("\t}")
...
func (g *gen) copyFn(ins []colInfo) {
	...
	g.p("func InsertAll(ctx context.Context, ex "+g.execType()+", rows []Row) (int64, error) {")
	g.p("\tif len(rows) == 0 {")
	g.p("\t\treturn 0, nil")
	g.p("\t}")
