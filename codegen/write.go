...
func (g *gen) insInsertNoReturn(ins []colInfo) {
	...
	g.p("\t\t\t// DO NOTHING on a target that cannot return: the statement has no")
	g.p("\t\t\t// MATCHED branch, so a row that was already there changes nothing")
	g.p("\t\t\t// and affects zero. That is the SUCCESS case of an idempotent")
	g.p("\t\t\t// insert, and it has to report the same error the returning")
	g.p("\t\t\t// targets do — a caller whose contract changed with the dialect")
	g.p("\t\t\t// would retry forever on one of them.")
	g.p("\t\t\treturn Row{}, runtime.ErrConflict")
	g.p("\t\t}")
...
func (g *gen) insType(ins []colInfo) {
	...
	g.p("func (n *Ins) Insert(ctx context.Context, ex %s) (Row, error) {", g.execType())
	g.p("\tif n.set == 0 {")
	...
	g.p("\tif err := scan(rows.%s(), &out, &sl); err != nil {", g.dec.rowsAccessor())
	g.p("\t\treturn out, err")
	g.p("\t}")
...
func (g *gen) insertFn(ins []colInfo) {
	...
	g.p("func Insert(ctx context.Context, ex %s, r *Row) error {", g.execType())
	...
	g.p("\tvar sl runtime.Slab")
	g.p("\tif err := scan(rows.%s(), r, &sl); err != nil {", g.dec.rowsAccessor())
	g.p("\t\treturn err")
	g.p("\t}")
	g.p("\treturn rows.Err()")
	g.p("}")
...
func (g *gen) updateFn(upd, pk []colInfo) {
	...
	// The capacity counts what may actually be appended: the keys, a version
	// frag, and the liveness test only when there is one. Unconditionally
	// reserving the extra slot would change the generated bytes of every table
	// that did NOT opt in, which is not what opt-in means.
	extra := 1
	if g.t.SoftDeletes() {
		extra++
	}
	g.p("\twhere := make([]runtime.Frag, 0, %d)", len(pk)+extra)
	g.p("\twhere = append(where, pkFrags[:]...)")
	if hasVersion {
		g.p("\twhere = append(where, versionFrag)")
	}
	if g.t.SoftDeletes() {
		// An UPDATE that finds a deleted row is a caller acting on something
		// they were told no longer exists — every read in this package agrees
	...
	g.p("func (m *Mut) Update(ctx context.Context, ex %s) error {", g.execType())
	...
	if refresh {
		...
		g.p("\tvar sl runtime.Slab")
		g.p("\tif err := scan(rows.%s(), &m.row, &sl); err != nil {", g.dec.rowsAccessor())
		g.p("\t\treturn err")
		g.p("\t}")
		g.p("\tif err := rows.Err(); err != nil {")
		g.p("\t\treturn err")
...
//
// Delete keeps its name and its meaning — "this row is gone as far as the
// application is concerned" — because renaming it would leave `Delete` free for
// someone to add back later meaning something else. What changes is that the
// row survives, and that two more functions exist to say the things you can now
...
func (g *gen) softDeleteFns(pk []colInfo) {
	...
	sigs, calls := joinStr(sig, ", "), joinStr(call, ", ")

	// Marking an already-marked row must not look like success, so the
	// statement finds only live rows and a second Delete reports ErrNoRow —
	// the same answer a hard delete gives for a row that is not there.
	...
	g.p("var softDeleteSQL = %s(%s, []runtime.Section{", g.spliceFn(), lit(g.lw.SoftDeleteSet(g.t.Name, col)))
	g.p("\t{Lead: %q, Sep: %q, Frags: append(pkFrags[:], runtime.Frag{A: %s})},", g.lw.WhereLead, g.lw.WhereSep, lit(alive))
	g.p("}, %s).SQL", g.spliceTail())
	g.p("")
	g.p("// Delete marks one row deleted. The row stays in the table and keeps its")
	g.p("// keys; every read generated in this package stops returning it.")
	g.p("//")
	g.p("// A row that was already deleted is runtime.ErrNoRow, not success — the")
	g.p("// same answer a hard delete gives for a row that is not there.")
	g.p("func Delete(ctx context.Context, ex "+g.execType()+", %s) error {", sigs)
	g.p("\tn, err := ex.Exec(ctx, softDeleteSQL, []any{%s})", calls)
	g.p("\tif err != nil {")
	g.p("\t\treturn err")
	g.p("\t}")
	g.p("\tif n == 0 {")
	...
	g.p("}")
	g.p("")
	g.p("var hardDeleteSQL = %s(deletePrefix, []runtime.Section{", g.spliceFn())
	...
	g.p("// function in this package that destroys data, which is why it is not")
	g.p("// spelled Delete.")
	g.p("func HardDelete(ctx context.Context, ex "+g.execType()+", %s) error {", sigs)
	g.p("\tn, err := ex.Exec(ctx, hardDeleteSQL, []any{%s})", calls)
	g.p("\tif err != nil {")
	g.p("\t\treturn err")
	g.p("\t}")
	g.p("\tif n == 0 {")
	...
	g.p("}")
	g.p("")
	g.p("var restoreSQL = %s(%s, []runtime.Section{", g.spliceFn(), lit(g.lw.RestoreSet(g.t.Name, col)))
	...
	g.p("// restoring something that was never gone is a caller believing something")
	g.p("// untrue about the row, and reporting success would leave them believing it.")
	g.p("func Restore(ctx context.Context, ex "+g.execType()+", %s) error {", sigs)
	g.p("\tn, err := ex.Exec(ctx, restoreSQL, []any{%s})", calls)
	g.p("\tif err != nil {")
	g.p("\t\treturn err")
	g.p("\t}")
	g.p("\tif n == 0 {")
...
func (g *gen) deleteFn(pk []colInfo) {
	...
	g.p("var deleteSQL = %s(deletePrefix, []runtime.Section{", g.spliceFn())
	g.p("\t{Lead: %q, Sep: %q, Frags: pkFrags[:]},", g.lw.WhereLead, g.lw.WhereSep)
	g.p("}, %s).SQL", g.spliceTail())
	g.p("")
	...
	g.p("func Delete(ctx context.Context, ex "+g.execType()+", %s) error {", joinStr(sig, ", "))
...
func (g *gen) batchOps(ins, upd, pk []colInfo) {
	...
	// The batch path has to make the SAME choice as Delete. It is the easiest
	// one to miss — a queued DeleteOp that hard-deletes on a soft-delete table
	// destroys rows the caller believed were recoverable, and nothing in the
	// call site says which of the two it got.
	deleteStmt := "deleteSQL"
	if g.t.SoftDeletes() {
		deleteStmt = "softDeleteSQL"
		g.p("// DeleteOp marks the row deleted, exactly as Delete does: a queued")
		g.p("// delete and a direct one must not differ in whether the row survives.")
		g.p("// HardDeleteOp is the queueable form of HardDelete.")
	} else {
	...
	g.p("\treturn runtime.BatchOp{SQL: %s, Args: []any{%s}}", deleteStmt, joinStr(call, ", "))
	g.p("}")
	g.p("")
	if g.t.SoftDeletes() {
		g.p("// HardDeleteOp removes the row for real, as a queueable statement.")
		g.p("func HardDeleteOp(%s) runtime.BatchOp {", joinStr(sig, ", "))
		g.p("\treturn runtime.BatchOp{SQL: hardDeleteSQL, Args: []any{%s}}", joinStr(call, ", "))
		g.p("}")
		g.p("")
		g.p("// RestoreOp clears the mark, as a queueable statement.")
		g.p("func RestoreOp(%s) runtime.BatchOp {", joinStr(sig, ", "))
		g.p("\treturn runtime.BatchOp{SQL: restoreSQL, Args: []any{%s}}", joinStr(call, ", "))
		g.p("}")
		g.p("")
	}
...
func (g *gen) copyFn(ins []colInfo) {
	...
	g.p("func InsertAll(ctx context.Context, ex %s, rows []Row) (int64, error) {", g.execType())
	g.p("\tif len(rows) == 0 {")
	g.p("\t\treturn 0, nil")
	g.p("\t}")
