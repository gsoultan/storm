...
// execType is the executor parameter type every generated call takes:
// runtime.Executor for an ordinary table, shard.Bound for a sharded one.
//
// This one function is the whole compile-time half of sharding. A sharded
// model's rows live on several databases and nothing in any of them knows the
// others exist, so "which shard" cannot be checked by a constraint, recovered
// from an error, or noticed in a result — a query aimed at the wrong one
// returns rows, just not the right ones. Making the parameter a type that
// only shard.Set and shard.Pin can produce moves that from a class of bug to
// a class of compile error.
//
// A union has no driving table (ADR-0008), so g.t is nil there and the answer
// is the unsharded one: a union over sharded tables is refused at generate
// time rather than given a Bound it could not honour for every branch.
func (g *gen) execType() string {
	if g.t != nil && g.t.Sharded() {
		return "shard.Bound"
	}
...
func (g *gen) header() {
	...
	if g.t != nil && g.t.Sharded() {
		g.p("\t%q", g.o.Import+"/runtime/shard")
	}
...
func (g *gen) compile() {
	...
	g.p("\t}")
	g.p("\tcs[0][0], cs[0][1] = cache, offsetCache")
	g.p("\treturn cs")
	g.p("}()")
	...
	g.p("func stmtFor(toks []runtime.Tok, withOffset bool, lock uint8) *runtime.Stmt {")
	g.p("\ti, suffix := 0, limitSuffix")
	g.p("\tif withOffset {")
	g.p("\t\ti, suffix = 1, limitOffsetSuffix")
	g.p("\t}")
	g.p("\tc := lockCaches[lock][i]")
	g.p("\tsuffix += lockSuffix[lock]")
	...
	g.p("\tvar buf [%d]runtime.Tok", g.streamBuf())
	g.p("\treturn stmtFor(q.stream(&buf), q.offset > 0, q.lock).SQL")
	g.p("}")
	g.p("")
}
...
func (g *gen) scanner() {
	...
	g.p("func Scan(rv %s, r *Row, sl *runtime.Slab) error { return scan(rv, r, sl) }", g.dec.rowsType())
	g.p("")
	...
	g.p("func scan(rv %s, r *Row, sl *runtime.Slab) error {", g.dec.rowsType())
...
func (g *gen) terminals() {
	...
	g.p("func (q Query) All(ctx context.Context, ex %s, dst []Row) ([]Row, error) {", g.execType())
	g.p("\tvar sl runtime.Slab")
	g.p("\treturn q.AllInto(ctx, ex, dst, &sl)")
	g.p("}")
	g.p("")
	g.p("// AllInto lets the caller own the string arena.")
	g.p("func (q Query) AllInto(ctx context.Context, ex %s, dst []Row, sl *runtime.Slab) ([]Row, error) {", g.execType())
	g.p("\tif err := q.Err(); err != nil {")
	g.p("\t\treturn dst, err")
	g.p("\t}")
	g.p("\tvar buf [%d]runtime.Tok", g.streamBuf())
	g.p("\tst := stmtFor(q.stream(&buf), q.offset > 0, q.lock)")
	g.p("\tif st.Err != nil {")
	...
	g.p("\t\tif err := scan(rows.%s(), &dst[len(dst)-1], sl); err != nil {", g.dec.rowsAccessor())
	g.p("\t\t\treturn dst, err")
	g.p("\t\t}")
	g.p("\t}")
	...
	g.p("func (q Query) One(ctx context.Context, ex %s) (Row, bool, error) {", g.execType())
	g.p("\tvar sl runtime.Slab")
	g.p("\tvar buf [1]Row")
	...
	g.p("func (q Query) Count(ctx context.Context, ex %s) (int64, error) {", g.execType())
	g.p("\tif err := q.Err(); err != nil {")
	g.p("\t\treturn 0, err")
	g.p("\t}")
	g.p("\tif q.lock != 0 {")
	g.p("\t\treturn 0, errCountLocked")
	g.p("\t}")
	g.p("\tvar buf [%d]runtime.Tok", g.streamBuf())
	...
	g.p("\trows, err := %s", g.dec.rowsFrom("ex.Query(ctx, st.SQL, q.bindPreds(b))"))
	g.p("\tif err != nil {")
	g.p("\t\treturn 0, err")
	g.p("\t}")
	g.p("\tdefer rows.Close()")
	g.p("\tvar n int64")
	g.p("\tif rows.Next() {")
	g.p("\t\tn = %s(rows.%s()[0])", g.dec.q("Int8"), g.dec.rowsAccessor())
	g.p("\t}")
	...
	g.p("func (q Query) Exists(ctx context.Context, ex %s) (bool, error) {", g.execType())
	g.p("\tif err := q.Err(); err != nil {")
	g.p("\t\treturn false, err")
	g.p("\t}")
	g.p("\tif q.lock != 0 {")
	g.p("\t\treturn false, errExistsLocked")
	g.p("\t}")
	g.p("\tvar buf [%d]runtime.Tok", g.streamBuf())
	...
	g.p("\tvar buf [%d]runtime.Tok", g.streamBuf())
	g.p("\treturn stmtFor(q.stream(&buf), q.offset > 0, q.lock).SQL, q.bind(b)")
	g.p("}")
	g.p("")
}
