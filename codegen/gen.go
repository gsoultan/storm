...
func (g *gen) terminals() {
	...
	g.p("func (q Query) All(ctx context.Context, ex "+g.execType()+", dst []Row) ([]Row, error) {")
	g.p("\tvar sl runtime.Slab")
	g.p("\treturn q.AllInto(ctx, ex, dst, &sl)")
	g.p("}")
	g.p("")
	g.p("// AllInto lets the caller own the string arena.")
	g.p("func (q Query) AllInto(ctx context.Context, ex "+g.execType()+", dst []Row, sl *runtime.Slab) ([]Row, error) {")
	...
	g.p("func (q Query) One(ctx context.Context, ex "+g.execType()+") (Row, bool, error) {")
	g.p("\tvar sl runtime.Slab")
	g.p("\tvar buf [1]Row")
	...
	g.p("func (q Query) Count(ctx context.Context, ex "+g.execType()+") (int64, error) {")
	g.p("\tif err := q.Err(); err != nil {")
	g.p("\t\treturn 0, err")
	g.p("\t}")
	g.p("\tif q.lock != 0 {")
	...
	g.p("func (q Query) Exists(ctx context.Context, ex "+g.execType()+") (bool, error) {")
	g.p("\tif err := q.Err(); err != nil {")
